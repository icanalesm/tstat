...
const char *volume(const void *mixer)
{
	...
	mix = (struct mixer *)mixer;

	if ((aux = snd_mixer_open(&handle, 0)) < 0) {
		error("Mixer open error: %s", snd_strerror(aux));
		return NULL;
	}
	if ((aux = snd_mixer_attach(handle, mix->card)) < 0) {
		error("Mixer attach '%s' error: %s", mix->card, snd_strerror(aux));
		goto clean_handle;
	}
	if ((aux = snd_mixer_selem_register(handle, NULL, NULL)) < 0) {
		error("Mixer register error: %s", snd_strerror(aux));
		goto clean_handle;
	}
 	if ((aux = snd_mixer_load(handle)) < 0) {
		error("Mixer load '%s' error: %s", mix->card, snd_strerror(aux));
		goto clean_handle;
	}
	...
	if (!(elem = snd_mixer_find_selem(handle, sid))) {
		error("Unable to find '%s',%i in card '%s'", mix->name,
		      mix->index, mix->card);
		goto clean_sid;
	}
